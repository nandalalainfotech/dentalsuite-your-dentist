...
import { useSupport } from '../../features/patient/support/support.hooks';
import type { SupportMessage, SupportTicket } from '../../features/patient/support/support.types';
...
export default function PracticeSupportForm() {
    ...
    // --- FORM STATES ---
    const [relatesTo, setRelatesTo] = useState('GENERAL');
    ...
    const statusFiltered = Array.isArray(tickets)
    ...
    const displayedTickets = statusFiltered.filter(ticket => {
        ...
        const matchesNumber =
            ticket.request_number?.toString().includes(search);
    ...
    return (
        <div className="h-[calc(100vh-140px)] bg-white rounded-lg flex gap-6 p-4">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-white">
                                ...
                                {messages.map(
                                    (
                                        ...
                                        const isOwnMessage =
                                            msg.sender_type?.toUpperCase() === 'PATIENT';
            ...
            {/* --- PREVIEW MODAL --- */}
            {previewImage && (
                <div className="fixed inset-0 z-[100] bg-gray-900/50 backdrop-blur-sm flex flex-col items-center justify-center p-10">
                    <button onClick={() => setPreviewImage(null)} className="absolute top-20 right-8 bg-black/60 hover:bg-black/80 p-3 rounded-full text-white transition-all">
                        <X className="w-6 h-6" />
                    </button>
                    <img src={previewImage} className="max-h-[70vh] w-auto rounded-2xl shadow-2xl border border-white/10" alt="" />
            ...
            {isCreateModalOpen && (
                <div className="fixed inset-0 z-[100] bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[80vh]">
                        ...
                        <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-100 flex justify-end gap-3">
                            ...
                            <button
                                ...
                                className="px-6 h-10 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold uppercase text-[12px] tracking-widest disabled:opacity-30 flex items-center gap-2 transition-all active:scale-95"
                            >
