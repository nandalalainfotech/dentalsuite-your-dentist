...
const initialAppointmentState: PracticeAppointmentState = {
  appointments: [],
...
const initialPatientState: PracticePatientState = {
  patients: [],
...
const initialSettingsState: PracticeSettingsState = {
  settings: null,
  isLoading: false,
  error: null,
};

const initialNotificationState: PracticeNotificationState = {
  notifications: [],
  unreadCount: 0,
  isLoading: false,
};

const initialDashboardState: PracticeDashboardState = {
  stats: null,
  isLoading: false,
  error: null,
};
...
const practiceSlice = createSlice({
  ...
  reducers: {
    ...
    confirmAppointment: (state, action: PayloadAction<string>) => {
      const appointment = state.appointments.appointments.find(
    ...
    setPatients: (state, action: PayloadAction<PracticePatientState['patients']>) => {
      state.patients.patients = action.payload;
      state.patients.isLoading = false;
    },
    addPatient: (state, action: PayloadAction<PracticePatientState['patients'][0]>) => {
      state.patients.patients.push(action.payload);
    },
    updatePatient: (state, action: PayloadAction<PracticePatientState['patients'][0]>) => {
      const index = state.patients.patients.findIndex(
        (p) => p.id === action.payload.id
      );
      if (index !== -1) {
        state.patients.patients[index] = action.payload;
      }
    },
    removePatient: (state, action: PayloadAction<string>) => {
      state.patients.patients = state.patients.patients.filter(
        (p) => p.id !== action.payload
      );
    },
    setSelectedPatient: (state, action: PayloadAction<unknown | null>) => {
      state.patients.selectedPatient = action.payload;
    },
    setPatientsLoading: (state, action: PayloadAction<boolean>) => {
      state.patients.isLoading = action.payload;
    },
    setSettings: (state, action: PayloadAction<PracticeSettingsState['settings']>) => {
      state.settings.settings = action.payload;
      state.settings.isLoading = false;
    },
    updateSettings: (state, action: PayloadAction<PracticeSettingsState['settings']>) => {
      state.settings.settings = action.payload;
    },
    ...
    setDashboardStats: (state, action: PayloadAction<PracticeDashboardState['stats']>) => {
