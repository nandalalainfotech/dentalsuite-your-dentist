...
interface AppliedCoupon extends Coupon {
    ...
    appliedToPlan?: 'pay_per_patient' | 'pay_per_month'; // From DB
...
export default function PracticeSubscription() {
    ...
    // FETCH COUPONS
    // =========================
    const { data: couponsData, refetch: refetchCoupons } = useQuery<{ coupons: Coupon[] }>(GET_ALL_COUPONS, {
        client: localClient,
    ...
    // =========================
    // GET ACTIVE COUPONS SEPARATED BY PLAN
    // =========================
    const practiceActiveCoupons = useMemo(() => {
        if (!coupons.length || !practiceId) {
            return {
        ...
        for (const coupon of coupons) {
            ...
            if (practiceData && practiceData.periods && Array.isArray(practiceData.periods)) {
                ...
                if (isStillValid) {
                    ...
                    // Separate based on appliedToPlan from DB
                    if (appliedCoupon.appliedToPlan === 'pay_per_patient') {
                        ...
                        // Fallback: if no appliedToPlan, check applies_to
    ...
    const [monthlyAddonEnabled, setMonthlyAddonEnabled] = useState(false);
    ...
    }, [subscription, isExpiredWithoutPending]);
    ...
    subscriptionEndDate.setUTCHours(0, 0, 0, 0);
    subscriptionEndDate.setUTCDate(subscriptionEndDate.getUTCDate() + 29);
    subscriptionEndDate.setUTCHours(23, 59, 59, 999);

    const nextPendingStartDate = new Date(subscriptionEndDate);
    nextPendingStartDate.setUTCDate(nextPendingStartDate.getUTCDate() + 1);
    nextPendingStartDate.setUTCHours(0, 0, 0, 0);
    ...
    const handleSaveSubscription = async () => {
        try {
            ...
            const targetPaymentType = monthlyAddonEnabled ? 'PAY_PER_MONTH' : 'PAY_PER_PATIENT';
            const currentPlan = subscription?.current_payment_type;
            const pendingPlan = subscription?.pending_payment_type;
            const planChanged = targetPaymentType !== currentPlan && targetPaymentType !== pendingPlan;

            // Only save the subscription plan if it actually changed
            if (planChanged) {
                await dispatch(
                    savePracticeSubscription({
                        practiceId,
                        paymentType: targetPaymentType,
                        monthly_addon_enabled: monthlyAddonEnabled,
                        subscription_start_date: subscriptionStartDate.toISOString(),
                        subscription_end_date: subscriptionEndDate.toISOString(),
                        pending_start_date: nextPendingStartDate.toISOString()
                    })
                ).unwrap();
            }

            // Apply validated coupon on save (no intermediate Apply step needed)
            if (validatedCoupon) {
                const planType = monthlyAddonEnabled ? 'pay_per_month' : 'pay_per_patient';
    ...
    const isSelected = (monthlyPlan: boolean) => monthlyAddonEnabled === monthlyPlan;
    const isActivated = (monthlyPlan: boolean) => monthlyPlan
        ? subscription?.current_payment_type === 'PAY_PER_MONTH'
        : subscription?.current_payment_type === 'PAY_PER_PATIENT';

    const hasPendingChange = (monthlyPlan: boolean) => monthlyPlan
    ...
    futureEndDate.setUTCDate(futureEndDate.getUTCDate() + 29);
    futureEndDate.setUTCHours(23, 59, 59, 999);
    ...
    const calculateDiscountedPrice = (originalPrice: number, coupon: AppliedCoupon, priceType: 'per_patient' | 'per_month') => {
        if (!coupon) return originalPrice;

        if (coupon.appliedTo === 'monthly' && priceType === 'per_patient') return originalPrice;
        if (coupon.appliedTo === 'per_patient' && priceType === 'per_month') return originalPrice;
    ...
    const getActivePrice = (priceType: 'per_patient' | 'per_month', planSpecificCoupons: AppliedCoupon[]) => {
        const originalPrice = priceType === 'per_patient'
            ? subscription?.current_price?.pay_per_patient_amount || 0
            : subscription?.current_price?.pay_per_month_amount || 0;

        let finalPrice = originalPrice;
        for (const coupon of planSpecificCoupons) {
            finalPrice = calculateDiscountedPrice(finalPrice, coupon, priceType);
        }
    ...
    const selectedPlanName = monthlyAddonEnabled ? 'Monthly Add-on' : 'Pay Per Patient';
    const selectedPlanCoupons = monthlyAddonEnabled ? practiceActiveCoupons.payPerMonth : practiceActiveCoupons.payPerPatient;
            ...
            <div className="max-w-5xl mx-auto">
                ...
                {loading ? (
                    ...
                    <>
                        <div className="bg-gray-50 rounded-2xl shadow-sm border border-gray-100 p-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div
                                    ...
                                    className={`cursor-pointer border rounded-2xl p-5 text-left transition-all ${isSelected(false)
                                ...
                                >
                                    ...
                                    {/* Show coupons for Pay Per Patient plan */}
                                    ...
                                    {!isActivated(false) && !hasPendingChange(false) && (
                                                ...
                                                {/* {practiceActiveCoupons.payPerPatient.length > 0 && (
                                                    <div className="flex items-center justify-between text-sm">
                                                        ...
                                                        <span className="font-semibold text-purple-600">
                                                            -${(subscription?.current_price?.pay_per_patient_amount || 0) - getActivePrice('per_patient', practiceActiveCoupons.payPerPatient)}
                                                        </span>
                                                    </div>
                                                )} */}
                                                <div className="flex items-center justify-between text-sm border-t border-green-200 pt-2 mt-1">
                                                    <span className="text-gray-700 font-semibold">Active Price</span>
                                                    <span className="font-bold text-green-700">
                                                        {/* ${getActivePrice('per_patient', practiceActiveCoupons.payPerPatient).toFixed(2)} */}
                                                        ${subscription?.current_price?.pay_per_patient_amount || 0}
                                ...
                                {/* PAY PER PATIENT + MONTHLY ADD ON CARD */}
                                <div
                                    onClick={() => {
                                    ...
                                    className={`cursor-pointer border rounded-2xl p-5 text-left transition-all ${isSelected(true)
                                        ? 'border-orange-500 bg-orange-50 ring-2 ring-orange-100'
                                        : 'border-gray-200 bg-white hover:border-orange-300'
                                        }`}
                                >
                                    ...
                                    {/* Show coupons for Monthly Add-on plan */}
                                    ...
                                    {!isActivated(true) && !hasPendingChange(true) && (
                                        <div className="mt-4 rounded-xl border border-gray-200 bg-white p-4 space-y-3">
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm text-gray-500">Start Date</span>
                                                <span className="text-sm font-semibold text-[#1a2b3c]">
                                        ...
                                        <div className="mt-4 p-3 rounded-xl bg-green-50 border border-green-200">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                {practiceActiveCoupons.payPerMonth.length > 0 && (
                                                    <>
                                                        <div className="flex items-center justify-between text-sm border-t border-green-200 pt-2 mt-1">
                                                            <span className="text-gray-700 font-semibold">Monthly Price with Coupon</span>
                                                            <span className="font-bold text-green-700">
                                                                ${getActivePrice('per_month', practiceActiveCoupons.payPerMonth).toFixed(2)}
                                                            </span>
                                                        </div>
                                                    </>
                                                )}
                        ...
                        {/* ==================================================== */}
                        {/* COUPON APPLICATION SECTION - Shows selected plan */}
                        {/* ==================================================== */}
                        ...
                        {/* ==================================================== */}
                        {/* SAVE SUBSCRIPTION BUTTON - BOTTOM */}
                        {/* ==================================================== */}
                        <div className="mt-6 flex justify-end">
                            <button
                                onClick={handleSaveSubscription}
                                disabled={saving}
                                className="px-6 py-2.5 rounded-lg bg-orange-500 hover:bg-orange-600 text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                            >
