...
export const RescheduleModal = ({ apt, isOpen, onClose, onConfirm, practitioners }: RescheduleModalProps) => {
  ...
  // Find the full object of the selected practitioner
  ...
  // ... (Date/Time Helpers remain unchanged) ...
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="flex-1 flex flex-col bg-white min-w-0 overflow-hidden">
             {/* ... (Existing Slots Code - Unchanged logic, just ensure JSX is closed properly) ... */}
                                        ...
                                        {dayData.slots.length === 0 ? (
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {dayData.slots.map((time) => {
                                                    ...
                                                    return (
                                                        <button key={`${dayData.dateStr}-${time}`} type="button" onClick={() => { setSelectedDate(dayData.dateStr); setSelectedTime(time); }} className={`px-3 py-2 rounded-lg text-sm font-medium transition-all border ${isSelected ? 'bg-blue-600 text-white border-blue-600 shadow-md ring-2 ring-blue-600/20' : 'bg-white text-gray-700 border-gray-200 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50'}`}>
            ...
            <div className="px-4 sm:px-6 py-4 border-t border-gray-200 bg-gray-50 flex-shrink-0">
                <div className="mb-4">
                    ...
                    <input type="text" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Reason..." className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:outline-none" />
                </div>
                <div className="flex flex-col sm:flex-row gap-3 sm:justify-between sm:items-center">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button onClick={handleConfirm} disabled={!selectedDate || !selectedTime || isLoading} className={`flex-1 sm:flex-none px-6 py-2.5 font-medium rounded-xl shadow-lg flex items-center justify-center gap-2 text-sm ${selectedDate && selectedTime && !isLoading ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>
